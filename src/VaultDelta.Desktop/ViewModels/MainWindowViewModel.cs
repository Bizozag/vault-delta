...
public sealed class MainWindowViewModel : INotifyPropertyChanged
{
    ...
    private sealed class PreviewPatchWorkflowService : IPatchWorkflowService
    {
        ...
        public ValueTask<VaultDelta.Application.Apply.BaselineValidationResult> ValidateAsync(VaultDelta.Application.Patches.PackageInspectionResult inspection, string targetRoot, IProgress<VaultDelta.Application.Apply.BaselineValidationProgress>? progress = null, CancellationToken cancellationToken = default) => ValueTask.FromException<VaultDelta.Application.Apply.BaselineValidationResult>(Error());
        public ValueTask<VaultDelta.Application.Apply.ApplyResult> ApplyAsync(string packagePath, string targetRoot, IReadOnlyList<VaultDelta.Application.Apply.ConflictResolution>? resolutions = null, IProgress<VaultDelta.Application.Apply.TransactionProgress>? progress = null, CancellationToken cancellationToken = default) => ValueTask.FromException<VaultDelta.Application.Apply.ApplyResult>(Error());
        public ValueTask<VaultDelta.Application.Apply.RollbackResult> RollbackAsync(string journalPath, IProgress<VaultDelta.Application.Apply.TransactionProgress>? progress = null, CancellationToken cancellationToken = default) => ValueTask.FromException<VaultDelta.Application.Apply.RollbackResult>(Error());
    }
}
