...
事务 Apply 不根据操作系统名称假定卷能力。`PlatformCapabilityProbe` 会在实际目标根与事务根执行可清理探针，验证写入、同卷目录移动和目标目录内覆盖式移动；同时记录大小写与 Unicode 文件名行为。Windows/macOS 语义适配器负责平台识别和链接判断。探测失败发生在目标锁、Journal 和业务文件写入之前。
