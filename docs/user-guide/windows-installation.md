...
```powershell
Get-FileHash .\VaultDelta-0.1.10-win-x64.zip -Algorithm SHA256
...
1. 将 ZIP 解压到本机普通目录，例如 `D:\Tools\VaultDelta-0.1.10-win-x64`。
...
0.1.9 在 OneDrive 同步目录内大量应用操作时，恢复记录可能被短暂占用并导致应用中断。0.1.10 对恢复记录写入增加重试，并在报错中标明具体阶段。若已有中断事务，先使用原 `journal.json` 恢复，再重新检查基线和应用更新包。
